...
from pip._internal.utils.misc import ARCHIVE_EXTENSIONS, SUPPORTED_EXTENSIONS, normalize_path, remove_auth_from_url
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        self.secure_origins = [("*", host, "*") for host in (trusted_hosts if trusted_hosts else [])]
    ...
    def get_formatted_locations(self):
        ...
        if self.index_urls and self.index_urls != [PyPI.simple_url]:
            lines.append(
                "Looking in indexes: {}".format(", ".join(remove_auth_from_url(url) for url in self.index_urls))
            )
        if self.find_links:
    ...
    def find_all_candidates(self, project_name):
        ...
        if dependency_versions:
            logger.debug(
                "dependency_links found: %s", ", ".join([version.location.url for version in dependency_versions])
            )
        ...
        if file_versions:
            ...
            logger.debug(
                "Local files found: %s", ", ".join([url_to_path(candidate.location.url) for candidate in file_versions])
            )
...
class HTMLPage(object):
    ...
    def iter_links(self):
        ...
        document = html5lib.parse(
            self.content, transport_encoding=_get_encoding_from_headers(self.headers), namespaceHTMLElements=False
        )
