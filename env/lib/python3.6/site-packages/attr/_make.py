...
def _make_attr_tuple_class(cls_name, attr_names):
    ...
    attr_class_template = ["class {}(tuple):".format(attr_class_name), "    __slots__ = ()"]
    if attr_names:
...
def _transform_attrs(cls, these, auto_attribs, kw_only):
    ...
    if these is not None:
        ...
        ca_list = sorted(
            ((name, attr) for name, attr in cd.items() if isinstance(attr, _CountingAttr)), key=lambda e: e[1].counter
        )
...
class _ClassBuilder(object):
    ...
    def _create_slots_class(self):
        ...
        base_names = self._base_names
        cd = {
            k: v for k, v in iteritems(self._cls_dict) if k not in tuple(self._attr_names) + ("__dict__", "__weakref__")
        }
    ...
    def add_cmp(self):
        ...
        cd["__eq__"], cd["__ne__"], cd["__lt__"], cd["__le__"], cd["__gt__"], cd["__ge__"] = (
...
def attrs(
    ...
    def wrap(cls):
        ...
        elif hash is True or (hash is None and cmp is True and frozen is True) and is_exc is False:
            builder.add_hash()
        else:
            if cache_hash:
                raise TypeError(
...
def _make_hash(attrs, frozen, cache_hash):
    ...
    def append_hash_computation_lines(prefix, indent):
        ...
        """
        method_lines.extend([indent + prefix + "hash((", indent + "        %d," % (type_hash,)])
    ...
    linecache.cache[unique_filename] = (len(script), None, script.splitlines(True), unique_filename)
...
def _add_cmp(cls, attrs=None):
    ...
    cls.__eq__, cls.__ne__, cls.__lt__, cls.__le__, cls.__gt__, cls.__ge__ = _make_cmp(attrs)  # noqa
        ...
        if slots is True:
            ...
            def fmt_setter(attr_name, value_var):
                if _is_slot_attr(attr_name, base_attr_map):
                    res = "_setattr('%(attr_name)s', %(value_var)s)" % {"attr_name": attr_name, "value_var": value_var}
                else:
            ...
            def fmt_setter_with_converter(attr_name, value_var):
                ...
                return tmpl % {"attr_name": attr_name, "value_var": value_var, "c": conv_name}
    ...
    for a in attrs:
        ...
        if a.init is False:
                ...
                if a.converter is not None:
                    lines.append(
                        fmt_setter_with_converter(
                            attr_name, "attr_dict['{attr_name}'].default".format(attr_name=attr_name)
                        )
                    )
                    conv_name = _init_converter_pat.format(a.name)
                    names_for_globals[conv_name] = a.converter
            ...
            if a.converter is not None:
                ...
                lines.append("else:")
                lines.append(
                    "    " + fmt_setter_with_converter(attr_name, init_factory_name + "({0})".format(maybe_self))
                )
                names_for_globals[_init_converter_pat.format(a.name)] = a.converter
            else:
                lines.append("    " + fmt_setter(attr_name, arg_name))
...
class Attribute(object):
    ...
    *Read-only* representation of an attribute.
    ...
    def convert(self):
        warnings.warn(
            "The `convert` attribute is deprecated in favor of `converter`.  " "It will be removed after 2019/01.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def from_counting_attr(cls, name, ca, type=None):
        ...
        inst_dict = {
            ...
            if k not in ("name", "validator", "default", "type", "convert")  # exclude methods and deprecated alias
        }
...
_a = [
    Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=True, hash=(name != "metadata"), init=True)
...
class _CountingAttr(object):
    ...
    *Internal* data structure of the attrs library.  Running into is most
    ...
    __attrs_attrs__ = tuple(
        Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=True, hash=True, init=True, kw_only=False)
        for name in ("counter", "_default", "repr", "cmp", "hash", "init")
    ) + (
        Attribute(
            name="metadata", default=None, validator=None, repr=True, cmp=True, hash=False, init=True, kw_only=False
        ),
    )
    cls_counter = 0
